...
Read `01-model/beginner/prompt.md` to get the comparison prompt.

Run the prompt against each model in sequence:

```bash
echo "=== Haiku ===" && time claude --model claude-haiku-4-5-20251001 --print "$(cat 01-model/beginner/prompt.md)"
echo "=== Sonnet ===" && time claude --model claude-sonnet-4-6 --print "$(cat 01-model/beginner/prompt.md)"
echo "=== Opus ===" && time claude --model claude-opus-4-7 --print "$(cat 01-model/beginner/prompt.md)"
```
